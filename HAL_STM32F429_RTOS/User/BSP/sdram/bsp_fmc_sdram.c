...
void SDRAM_Init(void)
{
    ...
    SdramHandler.Init.SDBank = FMC_SDRAM_BANK2;      /* 配置为FMC_BANK2 */
    SdramHandler.Init.ColumnBitsNumber = FMC_SDRAM_COLUMN_BITS_NUM_8;    /* 列宽 */
    SdramHandler.Init.RowBitsNumber = FMC_SDRAM_ROW_BITS_NUM_12;         /* 行地址宽度 */
    SdramHandler.Init.MemoryDataWidth = FMC_SDRAM_MEM_BUS_WIDTH_16;      /* 数据宽度 */
    SdramHandler.Init.InternalBankNumber = FMC_SDRAM_INTERN_BANKS_NUM_4; /* SDRAM内部的bank数量 */
    SdramHandler.Init.CASLatency = FMC_SDRAM_CAS_LATENCY_3;              /* CADLatency时钟个数 */
    SdramHandler.Init.WriteProtection = FMC_SDRAM_WRITE_PROTECTION_DISABLE;  /* 不使能写保护 */
    SdramHandler.Init.SDClockPeriod = FMC_SDRAM_CLOCK_PERIOD_2;          /* 配置同步时钟参数 */
    SdramHandler.Init.ReadBurst = FMC_SDRAM_RBURST_DISABLE;              /* 使能突发读模式 */
    SdramHandler.Init.ReadPipeDelay = FMC_SDRAM_RPIPE_DELAY_1;           /* 配置CAS个延迟多少了HCLK时钟后再读取数据*/
