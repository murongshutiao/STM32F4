...
HAL_StatusTypeDef Bsp_Eth_Init(void)
{
    ...
    MACAddr[5] = 0x00;
    heth.Instance = ETH;
    heth.Init.AutoNegotiation = ETH_AUTONEGOTIATION_ENABLE;   /* 网络自适应功能 */
    heth.Init.PhyAddress = LAN8720_PHY_ADDRESS;               /* 设置PHY地址 */
    heth.Init.MACAddr = &MACAddr[0];                          /* 设置MAC地址 */          
    heth.Init.RxMode = ETH_RXINTERRUPT_MODE;                  /* 设置为接收中断 */   
    heth.Init.ChecksumMode = ETH_CHECKSUM_BY_HARDWARE;        /* 硬件进行校验 */
    heth.Init.MediaInterface = ETH_MEDIA_INTERFACE_RMII;      /* RMII接口模式 */
    heth.Init.Speed = ETH_SPEED_100M;                         /* 速度 */
    heth.Init.DuplexMode = ETH_MODE_FULLDUPLEX;               /* 全双工模式 */
